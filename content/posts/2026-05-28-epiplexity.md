...
这个区分很重要。因为它意味着「可学习的结构」决定了信息的价值，单纯的「不可预测性」只提供了熵。
...
论文把这叫做「归纳」（Induction）。数据的生成者遵循前向规则，数据的学习者则承担从结果恢复结构的归纳过程。归纳的计算成本，可以远远超过生成的计算成本。

这就解释了一个一直困扰很多人的问题：为什么「预测下一个词」这么简单的训练目标，能让模型产生逻辑推理能力？

因为要准确预测下一个词，模型有时候必须做归纳推理。它必须从已有的线索中推断出隐藏的结构，才能做出正确的预测。预测任务施加的归纳压力由此塑造了推理能力。
...
结果是：图像数据的总信息量最大，但 epiplexity 最低。超过 99% 的图像信息是「随机的」，比如具体像素值的不可预测性。文本数据的总信息量低于图像，epiplexity 却远远更高。

这解释了 AI 圈一个长期的困惑：为什么 LLM 通过海量文本训练就能产生极强的通用泛化能力，能迁移到机器人控制、定理证明、时间序列预测等完全不同的领域，而早期的计算机视觉模型吃了几十亿张图片，却依然只能做分类和检测？

关键在于文本中可学习的结构占比远远更高。语言天然具有长程依赖、组合性和抽象推理结构，这些都是高 epiplexity 的来源；图像的大部分信息量则被像素级噪音占据。

有意思的是，当他们对图像做 VQ tokenization（把像素压缩成离散的语义 token）之后，图像的 epiplexity 显著提升。因为 tokenization 过程滤掉了像素级的噪音，让模型能够聚焦于更高层的语义结构。

这也许解释了为什么最近的多模态模型越来越倾向于把所有模态都转换成 token 序列：tokenization 本身能够过滤低层噪音、提升 epiplexity，让 Transformer 接触到更集中的可学习结构。

维果茨基提出过「最近发展区」的概念：学习最有效的区域，位于学习者当前能力和潜在能力之间。内容难度与当前能力适度错开，既能提供新结构，也让学习者有能力完成理解。

## 物理数据的epiplexity

如果说 epiplexity 给语言模型训练提供了一个新的数据价值视角，那么它对 physical AI 的启发可能是：

在物理系统中，原始观测往往同时包含两类东西：一类是不可迁移的细节，例如像素噪声、传感器误差、热涨落、网格伪影和单条混沌轨迹的相位信息；另一类是可迁移的结构，例如守恒律、对称性、因果关系、状态变量、尺度律、闭合关系和可组合的动力学机制。前者增加的是熵，后者才构成 epiplexity。真正有价值的物理数据，取决于它们能否迫使模型从观测中抽取可复用的物理结构，分辨率、规模和真实程度只是服务于这一目标的条件。

**高 epiplexity 的物理数据**会迫使模型学习可复用的物理结构，高分辨率、高精度和大规模只是可能的外在特征。例如，跨几何、跨边界条件、跨 Reynolds/Mach/Knudsen 数、跨材料参数的流体或分子模拟；从稀疏观测反推压力场、应力场、隐变量、边界条件或本构关系的逆问题数据；从微观 MD 轨迹中提取宏观状态变量、reaction coordinates、order parameters、transport coefficients 的多尺度数据；包含 action/intervention 的 embodied physics 数据，比如机器人推、抓、碰撞、滑动后的视觉—触觉—力觉—位姿序列。这些数据的共同点是：生成过程可能很简单，但对学习者来说，预测或压缩它们需要建立隐藏的状态表征、因果结构、对称性、守恒律和闭合关系。

**低 epiplexity 的物理数据**仍可能非常大、非常高维，甚至具有很高的 Shannon 熵，只是主要信息来自不可迁移的随机性或捷径。例如，单一几何、单一参数、单一初边值条件下的大量 CFD snapshot，模型可能只学到插值；超高分辨率湍流或 MD 轨迹中的微观热噪声和相位细节，对许多宏观任务并无可复用结构；被传感器噪声、网格伪影、数值耗散、渲染纹理主导的数据；或者标签/格式中存在捷径（shortcut），使模型绕开物理机制也能降低 loss。

Epiplexity这个概念对Physical AI研究的启发应该是：模型既要学习更多物理规律，也要学习什么样的表征能让物理规律显现出来。这也是我认为为什么Physics-Informed ML一定会逐渐过渡到Physical Representation Learning：前者试图让模型服从物理，后者进一步让模型发现“物理应当以什么形式被表示”。

这就像早期的专家系统和如今的LLM的对比，专家系统执着于制定规则让模型遵从，而LLM通过设计让模型学习到了语言的表征，迫使模型理解了**语义**，从而自然而然的能解决大量OOD问题。

所以，真正能够理解物理规律的世界模型，其核心目标应当是学习一种表征，使原始观测中的物理结构从噪声和细节中浮现出来，并自然吸收已有物理规律提供的先验。在好的表征下，复杂的高维物理现象应当被压缩为更少的状态变量、更稳定的因果关系、更清晰的对称性和更可迁移的动力学规律。
