...
但这里有一个默认前提：**复杂度可以作为可解释性的代理。** 根据Occam's Razor原则，这个前提大多数时候是成立的，但并不严格。表达式的长度和节点数只能解释一部分阅读难度，与人类科学建模时偏好的形式仍有距离。尤其在物理问题里，这个落差其实非常明显。一个包含少量三角、指数或嵌套分式的表达式，树长可能不大，但读起来可能非常拧；一个稍长的多项式、无量纲组合或者分层展开式，虽然节点更多，却往往更符合人的推理习惯。
...
第二层是**轻语义复杂度**。这类做法承认不同算子带给人的负担并不一样，因此尝试给函数符号加权。例如加减乘除、幂、log、exp、三角函数并不等权。Kommenda 等人的工作就明确讨论过：size-related complexity 描述表达式树的形状，对函数符号进行语义区分则可以进一步刻画内部运算的“认知难度”。PSE 这篇工作在实现中采用的，也正是这种精神：表面上说是“operator 数量的 proxy”，实际代码里已经是带权运算符复杂度了。

第三层才是我真正关心的：**人类可解释性本身。** 它超出了纯语法对象的范畴。人类在看一个公式时，会自然地受到许多因素影响：它是否容易心算或局部模拟，它是否可以分解成几个彼此可命名的块，它是否包含过多非算术操作，它是否反复嵌套某类函数，它是否和该领域常见的理论结构相似。换句话说，人类阅读公式时更像是在“读结构”，节点数量只是其中一个线索。

Virgolin 等人 2020 年的一篇工作，正是试图解决这个问题。他们把问题转向：**能不能直接从人的反馈里学一个 interpretability proxy？** 他们设计了一个关于数学公式的问卷，围绕 simulatability 和 decomposability 两种常见的 proxy of human interpretability 收集数据，再用学习到的模型去估计一个公式的人类可解释性，并把这个模型嵌回多目标遗传编程中。这个思路在我看来很重要，因为它至少说明了一件事：在符号回归里，“复杂度”可以从不可动摇的手工先验，转变为一个**被学习的偏好函数**。

所以，我们接下来更值得考虑的是：**人到底偏好什么样的公式？这个偏好能否被学习并在搜索中施加？** 这比继续寻找唯一合理的复杂度定义更接近问题核心。

从这个角度看，树大小、运算符数量、带权复杂度，都更像是一种廉价的、静态的、预先写死的 proxy。它们以低成本见长，与真实偏好仍有距离，适合作为起点；终点还需要更贴近人的判断。

# 2. RLHF

在LLM的Post-training领域，经常用到一种名为**RLHF**（Reinforcement Learning from Human Feedback，基于人类反馈的强化学习）的微调技术。

在强化学习的传统设定里，我们默认任务目标已经被清晰地写成了奖励函数。可现实里的许多目标复杂而含混，很难直接写成奖励函数。

早在2017年，Christiano 等人指出：许多任务的目标复杂、含糊、难以精确定义，手写 reward 往往会造成“奖励函数被优化了，但真正想要的行为没有出现”。于是他们让人比较两段行为片段哪个更符合偏好，再用这些比较数据训练 reward model，最后让策略去优化这个 learned reward。人的比较判断由此取代了直接编写 reward 的要求。
...
这不正是我们想要的吗？这套思想在符号回归里同样适用：直接从人对候选公式的偏好中学习一个 interpretability reward model，再让符号生成器去优化这个 learned reward，就能比手写复杂度函数更自然地表达“可解释性”。

# 3. 从 Virgolin 的 proxy 出发：能不能用 RLHF 去约束符号回归中的可解释性？

## 3.1 为什么这个方向是自然的

先看 Virgolin 的工作到底做了什么。它其实已经完成了 RLHF 思路中最关键的一半：**从人类反馈学习一个偏好代理。** 它当时采用的是“interpretable symbolic regression”叙事，后来流行的“alignment”视角可以为同一逻辑提供新的解释。它的核心逻辑是：
...
### 第一层：把问题表述为 preference learning，超越 complexity engineering

这一步最重要。由标注者在同一任务、相近精度的前提下，对两个候选公式回答一个更自然的问题，固定的手写复杂度函数也由此升级为可学习的偏好：
...
这一步可以直接采用 pairwise preference，无需求标注者给出精确分数。我觉得两两比较更可靠，因为“7 分和 8 分的差别是什么”很难说清，但“这两个里面哪个更顺眼”通常容易得多。

### 第二层：学一个 task-conditioned 的 interpretability model

我更赞成同时输入公式和任务上下文，因为公式是否好理解，经常是**任务相关**的。
...
所以，一个更合理的偏好模型应同时输入 expression tree 和任务上下文，例如变量名、量纲、物理语义标签、是否已经无量纲化、误差水平、甚至候选公式所在的 Pareto 邻域。我们要学习的目标可以写成 $p(\text{preferred}\mid f, \mathcal{T})$，把任务条件直接纳入对公式的偏好判断。
...
我觉得可行性是有的，但困难也很真实，涉及远超“算力够不够”的多层问题。

### 难点一：可解释性具有关系属性

一个公式的“好不好懂”往往取决于对谁而言、在什么任务里、和什么候选相比。对于熟悉傅里叶分析的人，正弦项很容易理解；对于做热工关联式的人，某些经验指数关系也很自然；而对另一类研究者，同样的形式可能完全陌生。

这意味着学习**偏好分布**更符合可解释性的关系属性，也比追求一个放之四海而皆准的“绝对解释性分数”更实际。如果做得更进一步，甚至可以考虑 personalized interpretability，也就是根据不同用户群体学习不同偏好模型。

### 难点二：公式有等价性，偏好数据会被表示法污染

数学表达式的一个麻烦在于，不同写法可能是同一个式子。比如因式分解前后、常数吸收前后、对数幂变换前后，表面长度和结构差别很大，但数学上几乎等价。缺少 canonicalization 或符号化简时，偏好学习很容易捕捉“排版习惯”，从而偏离真正的“结构偏好”。

因此，这类系统必须把等价类处理建成核心基础设施，让它贯穿训练和搜索过程。至少要尽可能做符号化简、常数标准化、树结构规范化，最好还能对“可被局部代换解释为同一结构”的公式做聚类。

### 难点三：人类反馈很贵，而且专家反馈更贵

因此，我更倾向于把小规模、高质量反馈作为第一步。符号回归的可解释性判断经常需要一定数学或领域背景，大规模众包很难保证质量。更现实的路线可能是：
...
这是我觉得最需要警惕的一点。真实的物理规律有时会显得复杂，其结构常常超出人类第一眼的审美。过强的可解释性偏好可能会把搜索推向过度简单、过度教科书式的区域，从而错过真实但稍显复杂的结构。
